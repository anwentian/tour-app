...
export default function Slider(options){
    ...
    this.width = this.banner.clientWidth;
...
Slider.prototype.initial = function(){
    for(let i =0;i<this.itemsCount;i++){
        this.carouselItem[i].style.left = i*this.width+"px";
        this.carouselItem[i].style.width = this.width+"px";
    }
    for(let i =0;i<this.itemsCount;i++){
...
Slider.prototype.autoPlay = function(){
    ...
    let transform = "translate3d("+-this.width*this.target+"px, 0px,0px)";
...
Slider.prototype.touchEvent = function(){
    ...
    this.banner.addEventListener("touchmove",function(ev){ 
        ...
        if( 0 < _this.currentX < 320){
            _this.carosel.style['-webkit-transform']="translate3d("+(-_this.width*_this.current+_this.dis)+"px, 0px,0px)";
            _this.carosel.style.transform = "translate3d("+(-_this.width*_this.current+_this.dis)+"px, 0px,0px)";
    ...
    this.banner.addEventListener("touchend",function(ev){
       clearTimeout(_this.timer1);
        ...
        _this.carosel.style.transform = "translate3d("+-(_this.width*_this.target)+"px, 0px,0px)";
...
Slider.prototype.indicatorHandle = function(){
    ...
    for (let i = 0;i<this.itemsCount;i++) {     
        this.indicators[i].addEventListener("click",function(){          
            _this.current=i;
            let transform = "translate3d("+-_this.width*_this.current+"px, 0px,0px)";           
            let duration = "300ms";
